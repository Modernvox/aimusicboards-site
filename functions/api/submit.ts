...
export async function onRequest(context: { request: Request; env: Env }) {
  ...
  // In manual mode, free submissions require broadcasting open
  if (mode !== "auto" && !manualOpen) {
    return bad("Submissions are only accepted during broadcasting.", 403);
  }
  ...
  // We expect an R2 object key like "uploads/<uuid>.mp3"
  // Allow legacy clients that might still send "track_url" as:
  ...
  // Dupe by object key while still active
  const dupeKey = await env.DB
    .prepare(
      "SELECT id FROM submissions WHERE track_url = ? AND status IN ('NEW','IN_REVIEW') LIMIT 1"
    )
    .bind(track_key)
    .first<{ id: string }>();
  if (dupeKey) return bad("Duplicate: that track is already in the queue.", 409);

  // Dupe by same artist+title while still active
  const dupeName = await env.DB
    .prepare(
    ...
    .first<{ id: string }>();
  if (dupeName) return bad("Duplicate: that artist + track title is already in the queue.", 409);

  const id = uuid();
  const created_at = nowIso();

  // ✅ AUTO mode: enforce FREE queue cap atomically
  if (mode === "auto") {
    const res = await env.DB.prepare(`
      ...
      SELECT
        ?, ?, ?, ?, ?, ?, ?, 0, 0, 'NEW'
      WHERE
        (SELECT COUNT(*) FROM submissions WHERE paid = 0 AND status IN ('NEW','IN_REVIEW')) < ?
    `)
      .bind(id, created_at, artist_name, track_title, genre, track_key, notes || null, maxQueue)
      .run();

    const changes = Number((res as any)?.meta?.changes ?? 0);
    if (changes !== 1) {
      return bad(`Free queue is full (${maxQueue}). Paid reviews are still available.`, 409);
    }

    return json({ ok: true, id });
  }

  // Manual mode insert (broadcasting already checked above)
